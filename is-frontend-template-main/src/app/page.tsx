...
const getMapaData = async(search: string) => {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/allCars`, options)

  return await response.json()
}

const updatePoint = async(id: string, latitude: number, longitude: number) => {
  "use server"
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/allCars/${id}`, options)
